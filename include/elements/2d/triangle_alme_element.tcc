...
void mpm::TriangleALMEElement<Tdim>::initialise_lme_connectivity_properties(
...
//! local coordinate
template <unsigned Tdim>
inline Eigen::VectorXd mpm::TriangleALMEElement<Tdim>::shapefn(
  ...
  //! Substitute the TriALME by the FEM shape funtion in the limit of the
  //! simplex
    ...
    if (r.norm() > tolerance) {
      ...
      while (!convergence) {
        ...
        if (r.norm() < tolerance) {
          ...
          console_->warn("Max number of iteration for particle in TriALME");
          convergence = true;
...
template <unsigned Tdim>
inline Eigen::MatrixXd mpm::TriangleALMEElement<Tdim>::grad_shapefn(
    const Eigen::Matrix<double, Tdim, 1>& xi,
    Eigen::Matrix<double, Tdim, 1>& lambda,
    const Eigen::Matrix<double, Tdim, Tdim>& deformation_gradient) const {
...
//! Compute dn_dx
template <unsigned Tdim>
inline Eigen::MatrixXd mpm::TriangleALMEElement<Tdim>::dn_dx(
...
template <unsigned Tdim>
inline std::vector<Eigen::MatrixXd> mpm::TriangleALMEElement<Tdim>::bmatrix(
    const VectorDim& xi, const Eigen::MatrixXd& nodal_coordinates,
    VectorDim& lambda, const MatrixDim& deformation_gradient) const {
...
template <unsigned Tdim>
inline Eigen::VectorXd mpm::TriangleALMEElement<Tdim>::shapefn_local(
...
inline Eigen::Matrix<double, Tdim, Tdim>
    mpm::TriangleALMEElement<Tdim>::jacobian(
        const VectorDim& xi, const Eigen::MatrixXd& nodal_coordinates,
        VectorDim& lambda, const MatrixDim& deformation_gradient) const {
...
template <unsigned Tdim>
inline Eigen::Matrix<double, Tdim, Tdim>
    mpm::TriangleALMEElement<Tdim>::jacobian_local(
        const VectorDim& xi, const Eigen::MatrixXd& nodal_coordinates,
        VectorDim& lambda, const MatrixDim& deformation_gradient) const {
  // Jacobian dx_i/dxi_j
...
inline Eigen::Matrix<double, Tdim, 1>
    mpm::TriangleALMEElement<Tdim>::natural_coordinates_analytical(
  ...
  throw std::runtime_error(
      "Analytical solution for TriALME<Tdim> has "
      "not been "
      "implemented");
  return xi;
}
