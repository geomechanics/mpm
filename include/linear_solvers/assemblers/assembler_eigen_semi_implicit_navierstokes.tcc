...
mpm::AssemblerEigenSemiImplicitNavierStokes<
  ...
  //! Logger
  std::string logger = "AssemblerEigenSemiImplicitNavierStokes::";
...
bool mpm::AssemblerEigenSemiImplicitNavierStokes<Tdim>::assemble_poisson_right(
  ...
  try {
    if (active_dof_ > 0) {
      ...
      for (auto node_itr = active_nodes.cbegin();
           node_itr != active_nodes.cend(); ++node_itr) {
        // Compute nodal intermediate force
        fluid_velocity.row(node_index) =
            (*node_itr)->velocity(fluid).transpose();
        node_index++;
      }
