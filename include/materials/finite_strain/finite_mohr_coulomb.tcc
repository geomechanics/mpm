...
mpm::FiniteMohrCoulomb<Tdim>::FiniteMohrCoulomb(unsigned id,
                                                const Json& material_properties)
    : FiniteElastoPlastic<Tdim>(id, material_properties) {
  try {
    // General parameters
    // Density
...
template <unsigned Tdim>
mpm::dense_map mpm::FiniteMohrCoulomb<Tdim>::initialise_state_variables() {
  const mpm::dense_map& elastic_state_vars =
      mpm::FiniteElastoPlastic<Tdim>::initialise_state_variables();
  mpm::dense_map plastic_state_vars = {
      // MC parameters
  ...
  // Concatenate state vars
  mpm::dense_map state_vars = elastic_state_vars;
  state_vars.insert(plastic_state_vars.begin(), plastic_state_vars.end());
...
//! Initialise state variables
template <unsigned Tdim>
std::vector<std::string> mpm::FiniteMohrCoulomb<Tdim>::state_variables() const {
  const std::vector<std::string> elastic_state_vars =
      mpm::FiniteElastoPlastic<Tdim>::state_variables();
  const std::vector<std::string> plastic_state_vars = {
      "yield_state", "phi", "psi",   "cohesion", "tension_cutoff",
      "epsilon",     "rho", "theta", "pdstrain"};

  // Concatenate state vars
  std::vector<std::string> state_vars;
  state_vars.reserve(elastic_state_vars.size() + plastic_state_vars.size());
  state_vars.insert(state_vars.end(), elastic_state_vars.begin(),
                    elastic_state_vars.end());
  state_vars.insert(state_vars.end(), plastic_state_vars.begin(),
                    plastic_state_vars.end());
  return state_vars;
}

//! Compute stress invariants
template <unsigned Tdim>
bool mpm::FiniteMohrCoulomb<Tdim>::compute_stress_invariants(
...
typename mpm::mohrcoulomb::FailureState
    mpm::FiniteMohrCoulomb<Tdim>::compute_yield_state(
...
template <unsigned Tdim>
void mpm::FiniteMohrCoulomb<Tdim>::compute_df_dp(
...
//! Compute stress
template <unsigned Tdim>
Eigen::Matrix<double, 6, 1> mpm::FiniteMohrCoulomb<Tdim>::compute_stress(
...
Eigen::Matrix<double, 6, 6>
    mpm::FiniteMohrCoulomb<Tdim>::compute_elastic_tensor(
...
template <unsigned Tdim>
Eigen::Matrix<double, 6, 6>
    mpm::FiniteMohrCoulomb<Tdim>::compute_elasto_plastic_tensor(
