...
mpm::DilatantPlasticModel<Tdim>::DilatantPlasticModel(
...
template <unsigned Tdim>
mpm::dense_map mpm::DilatantPlasticModel<Tdim>::initialise_state_variables() {
  mpm::dense_map state_vars = {
      // MC parameters
      // Yield state: 0: elastic, 1: shear, 2: apex, 3: apex partial plastic, 4:
      // separated
      {"yield_state", 0},
...
//! Initialise state variables
template <unsigned Tdim>
std::vector<std::string> mpm::DilatantPlasticModel<Tdim>::state_variables()
    const {
...
//! Compute stress
template <unsigned Tdim>
Eigen::Matrix<double, 6, 1> mpm::DilatantPlasticModel<Tdim>::compute_stress(
      ...
      // returned to yield surface
        ...
        if (!rate_dependent_) {
          ...
          while (iter < max_iter_) {
            ...
            double mu_d = mu_c + 3.0 * beta;

            // Check bound of friction
            bool friction_bound = true;
            if (mu_d < tolerance_) {
              friction_bound = false;
            }

            // If friction is positive and within the bound, solve for all three
            // unknowns
            if (friction_bound) {
              // Compute residuals
              ...
              const double dr3_dgamma =
                  bulk_modulus_ * 3.0 * dt * phi_d_m * beta;
            ...
            // If friction is negative, solve a reduced system of only two
            // unknowns
            else {
              Eigen::Matrix<double, 2, 1> res_mod_m;
              Eigen::Matrix<double, 2, 2> jac_mod_m;

              // Max plastic shear strain rate
              gamma_m = tau_tr / (eff_shear_modulus * dt);

              // Compute residuals
              res_mod_m(0) =
                  p_m - p_tr - 3.0 * dt * bulk_modulus_ * beta * gamma_m;
              res_mod_m(1) =
                  bulk_modulus_ * (phi_d_m - current_packing_fraction_dilation +
                                   3.0 * dt * phi_d_m * beta * gamma_m);

              // Check convergence based on residual norm
              if (res_mod_m.norm() < abs_tol_) break;
              if (iter == 0)
                initial_res_norm = res_mod_m.norm();
              else {
                if (res_mod_m.norm() / initial_res_norm < rel_tol_) break;
              }
              ...
              // Compute Jacobian
              jac_mod_m(0, 0) = dr2_dp;
              jac_mod_m(0, 1) = dr2_dphi_d;
              jac_mod_m(1, 0) = dr3_dp;
              jac_mod_m(1, 1) = dr3_dphi_d;

              // Solve for increment in transformed space
              Eigen::Matrix<double, 2, 1> delta_lambda_mod_m =
                  jac_mod_m.inverse() * (-res_mod_m);

              // Update unknowns in transformed space
              lambda_m(0) += delta_lambda_mod_m(0);
              lambda_m(1) = gamma_m;
              lambda_m(2) += delta_lambda_mod_m(1);
              ...
              // Check convergence based on solution norm
              if (delta_lambda_mod_m.norm() < abs_tol_) {
                break;
              }
            }
        ...
        // Rate-dependent algorithm
          ...
          if (std::abs(dilation_scaling_) <
            ...
            while (iter < max_iter_) {
              ...
              // Compute Jacobian
              jac_m(0, 0) = 2.0 * lambda_m(0) * dr1_dp;
              jac_m(0, 1) = dr1_dgamma;
              jac_m(0, 2) = dr1_dphi_d;
              jac_m(1, 0) = 2.0 * lambda_m(0) * dr2_dp;
              jac_m(1, 1) = dr2_dgamma;
              jac_m(1, 2) = dr2_dphi_d;
              jac_m(2, 0) = 2.0 * lambda_m(0) * dr3_dp;
              jac_m(2, 1) = dr3_dgamma;
              jac_m(2, 2) = dr3_dphi_d;
...
Eigen::Matrix<double, 6, 6>
    mpm::DilatantPlasticModel<Tdim>::compute_elastic_tensor() {
  // Shear modulus
...
template <unsigned Tdim>
Eigen::Matrix<double, 6, 6>
    mpm::DilatantPlasticModel<Tdim>::compute_visco_tensor(double dt) {
  // Viscosity
...
template <unsigned Tdim>
Eigen::Matrix<double, 6, 6>
    mpm::DilatantPlasticModel<Tdim>::compute_elasto_plastic_tensor(
        const Vector6d& stress, const Vector6d& dstrain,
        const ParticleBase<Tdim>* ptr, mpm::dense_map* state_vars, double dt,
        double lin_v, double lin_a, bool hardening) {

  // Get yield type after return mapping algorithm
  mpm::dpm::FailureState yield_type =
      yield_type_.at(int((*state_vars).at("yield_state")));

  // Return the elastic consitutive tensor in elastic state
  if (yield_type == mpm::dpm::FailureState::Elastic) {
    const Matrix6x6 de = this->compute_elastic_tensor();
    const Matrix6x6 dv = this->compute_visco_tensor(dt);
    return de + dv;
  }

  // Return zero tensor matrix in apex, partial plastic, and separated state
  if ((yield_type == mpm::dpm::FailureState::Apex) ||
      (yield_type == mpm::dpm::FailureState::PartialPlastic) ||
      (yield_type == mpm::dpm::FailureState::Separated)) {
