...
mpm::MohrCoulombSFD<Tdim>::MohrCoulombSFD(unsigned id,
...
template <unsigned Tdim>
mpm::dense_map mpm::MohrCoulombSFD<Tdim>::initialise_state_variables() {
...
template <unsigned Tdim>
bool mpm::MohrCoulombSFD<Tdim>::compute_elastic_tensor() {
  // Shear modulus
...
template <unsigned Tdim>
bool mpm::MohrCoulombSFD<Tdim>::compute_stress_invariants(
...
template <unsigned Tdim>
typename mpm::mohrcoulombsfd::FailureState
    mpm::MohrCoulombSFD<Tdim>::compute_yield_state(
  ...
  auto yield_type = mpm::mohrcoulombsfd::FailureState::Elastic;
  ...
  if ((*yield_function)(0) > Tolerance && (*yield_function)(1) > Tolerance) {
    ...
    if (h > std::numeric_limits<double>::epsilon())
      yield_type = mpm::mohrcoulombsfd::FailureState::Tensile;
    // Shear
    else
      yield_type = mpm::mohrcoulombsfd::FailureState::Shear;
  }
  // Shear failure
  if ((*yield_function)(0) < Tolerance && (*yield_function)(1) > Tolerance)
    yield_type = mpm::mohrcoulombsfd::FailureState::Shear;
...
template <unsigned Tdim>
void mpm::MohrCoulombSFD<Tdim>::compute_df_dp(
    mpm::mohrcoulombsfd::FailureState yield_type,
  ...
  if (yield_type == mpm::mohrcoulombsfd::FailureState::Tensile) {
...
//! Compute stress
template <unsigned Tdim>
Eigen::Matrix<double, 6, 1> mpm::MohrCoulombSFD<Tdim>::compute_stress(
  ...
  if (yield_type_trial == mpm::mohrcoulombsfd::FailureState::Elastic)
    return trial_stress;
  //-------------------------------------------------------------------------
  ...
  if (yield_type_trial == mpm::mohrcoulombsfd::FailureState::Tensile)
    yield_trial = yield_function_trial(0);
  if (yield_type_trial == mpm::mohrcoulombsfd::FailureState::Shear)
  ...
  if (yield_type == mpm::mohrcoulombsfd::FailureState::Shear)
