...
//! \details Implementation based on the provided PM4Sand_claude_2.cpp, adapted for MPM.
//! Only valid for 2D Plane Strain problems.
//! \tparam Tdim Dimension (should be 2 for intended use, but framework might pass 3)
...
class PM4Sand : public InfinitesimalElastoPlastic<Tdim> {
 public:
  ...
  using Matrix3d = Eigen::Matrix3d; // For internal 3D calculations (representing 2D plane strain)
  ...
  //! Initialise material properties before analysis (if needed beyond constructor)
  ...
  Matrix6x6 compute_elasto_plastic_tensor(
      ...
      const ParticleBase<Tdim>* ptr,
      mpm::dense_map* state_vars,double dt, bool hardening = true) override;
 ...
 private:
  ...
  void getElasticModuli(const Matrix3d& sigma, double& K, double& G, mpm::dense_map* state_vars);
  //! Get elastic moduli (K, G, Mcur, zcum) - full version
  void getElasticModuli(const Matrix3d& sigma, double& K, double& G, double& Mcur, const double& zcum, mpm::dense_map* state_vars); // zcum is input

  //! Get the normal to the yield surface
  Matrix3d getNormalToYield(const Matrix3d& stress, const Matrix3d& alpha, mpm::dense_map* state_vars);

  //! Get the yield function value
  double getF(const Matrix3d& stress, const Matrix3d& alpha, mpm::dense_map* state_vars);

  //! Get state dependent parameters (n, D, R, K_p, etc.)
  void getStateDependent(const Matrix3d& stress, const Matrix3d& alpha,
  ...
  void stressCorrection(Matrix3d& nextStress, Matrix3d& nextAlpha,const Matrix3d& stress_prev,
                        const Matrix3d& alphaIn, const Matrix3d& alphaInP,
                        const Matrix3d& curFabric, double& nextVoidRatio, mpm::dense_map* state_vars);
  ...
  // void elasticIntegrator(const Matrix3d& dEpsilon, mpm::dense_map* state_vars);

  //! Update state variables map (internal helper, called by compute_stress)
  // void updateStateVariablesMap(const Matrix3d& stress, const Matrix3d& alpha, ... , mpm::dense_map* state_vars);
  ...
  inline double tensorNorm(const Matrix3d &A) const {
      ...
      // return std::sqrt((A.array() * A.array()).sum()); // Equivalent manual calculation
  }
   ...
   inline double macauley(double x) const {
      return (x > 0.0) ? x : 0.0;
   }
   inline double macauleyIndex(double x) const {
  ...
  double m_emax;
  double m_emin;
  // double m_e_init; // Calculated and stored in state_vars["void_ratio"] initially
  double m_nb;
  double m_nd;
...
// static constexpr double c_maxStrainInc = 1.0e-5; // Handled by MPM framework timestep?
...
//===========================================================================
// Note: Helper functions for accessing state variables (get_scalar, set_scalar, etc.)
// are typically implemented as free functions in an anonymous namespace
// within the .cpp/.tcc file to avoid polluting the class interface.
