...
inline double get_scalar(const mpm::dense_map* state_vars, const std::string& key) {
    try {
        ...
        throw std::runtime_error("[PM4Sand::get_scalar] State variable not found: " + key);
    } catch (const std::exception& e) {
         throw std::runtime_error("[PM4Sand::get_scalar] debug accessing key '" + key + "': " + e.what());
    }
}

// Helper to set a scalar value
inline void set_scalar(mpm::dense_map* state_vars, const std::string& key, double value) {
    // Ensure state_vars is not null
    if (!state_vars) {
...
inline Eigen::Matrix3d get_tensor(const mpm::dense_map* state_vars, const std::string& base_key) {
    Eigen::Matrix3d tensor = Eigen::Matrix3d::Zero();
    try {
        // Ensure state_vars is not null
        if (!state_vars) {
             throw std::runtime_error("[PM4Sand::get_tensor] state_vars map is null for key: " + base_key);
        }
        ...
        // tensor(0, 2) = tensor(2, 0); // zx symmetry
         ...
         throw std::runtime_error("[PM4Sand::get_tensor] Tensor state variable component not found for base key: " + base_key);
     } catch (const std::exception& e) {
         throw std::runtime_error("[PM4Sand::get_tensor] debug accessing tensor components for key '" + base_key + "': " + e.what());
    }
    return tensor;
}

// Helper to set a 3x3 tensor into state_vars
inline void set_tensor(mpm::dense_map* state_vars, const std::string& base_key, const Eigen::Matrix3d& tensor) {
     // Ensure state_vars is not null
    if (!state_vars) {
         throw std::runtime_error("[PM4Sand::set_tensor] state_vars map is null for key: " + base_key);
    }
...
template <unsigned Tdim>
 void PM4Sand<Tdim>::initialise_parameters(const Json& props) {
     // Use the pattern from Cycliq: Try reading with .at(), use default if missing.
     // We can achieve this using contains() check before accessing with .at()
     // or by catching the exception from .at(). Using contains() is often cleaner.
     ...
     try {
         ...
         if (props.contains("Dr")) {
             ...
             this->console_->debug("Material {}: Parameter 'Dr' not found in JSON, using default: {}", this->id_, m_Dr);
         }
         ...
         if (props.contains("G0")) {
             ...
             this->console_->debug("Material {}: Parameter 'G0' not found in JSON, using default: {}", this->id_, m_G0);
         }
         ...
         if (props.contains("hpo")) {
             ...
             this->console_->debug("Material {}: Parameter 'hpo' not found in JSON, using default: {}", this->id_, m_hpo);
         }
         ...
         if (props.contains("P_atm")) {
             ...
             this->console_->debug("Material {}: Parameter 'P_atm' not found in JSON, using default: {}", this->id_, m_P_atm);
         }
         ...
         if (props.contains("phi_cv")) {
             ...
             this->console_->debug("Material {}: Parameter 'phi_cv' not found in JSON, using default: {}", this->id_, phi_cv_deg);
         }
         if (phi_cv_deg < 0) phi_cv_deg = 33.0; // Ensure non-negative, use default if input is negative
         ...
         if (props.contains("h0")) {
             ...
             if (m_h0 < 0) {
                 ...
                 this->console_->debug("Material {}: Parameter 'h0' was < 0, calculating default: {}", this->id_, m_h0);
             }
         } else {
             m_h0 = std::max(0.3, (0.25 + m_Dr) / 2.0); // Default calculation
             this->console_->debug("Material {}: Parameter 'h0' not found in JSON, calculating default: {}", this->id_, m_h0);
         }
         ...
         if (props.contains("emax")) {
             ...
             this->console_->debug("Material {}: Parameter 'emax' not found in JSON, using default: {}", this->id_, m_emax);
         }
         ...
         if (props.contains("emin")) {
             ...
             this->console_->debug("Material {}: Parameter 'emin' not found in JSON, using default: {}", this->id_, m_emin);
         }
         // Ensure emax > emin
          if (m_emax <= m_emin) {
              this->console_->debug("Material {}: emax ({}) must be greater than emin ({}).", this->id_, m_emax, m_emin);
         ...
         if (props.contains("nb")) {
             ...
             this->console_->debug("Material {}: Parameter 'nb' not found in JSON, using default: {}", this->id_, m_nb);
         }
         ...
         if (props.contains("nd")) {
             ...
             this->console_->debug("Material {}: Parameter 'nd' not found in JSON, using default: {}", this->id_, m_nd);
         }
         ...
         if (props.contains("Ado")) {
             ...
             this->console_->debug("Material {}: Parameter 'Ado' not found in JSON, will calculate based on initial state.", this->id_);
         }
         ...
         if (props.contains("z_max")) {
             ...
             m_z_max = -1.0; // Indicate default calculation needed later
              this->console_->debug("Material {}: Parameter 'z_max' not found in JSON, will calculate based on initial state.", this->id_);
         }
         ...
         if (props.contains("cz")) {
             ...
             this->console_->debug("Material {}: Parameter 'cz' not found in JSON, using default: {}", this->id_, m_cz);
         }
         ...
         if (props.contains("ce")) {
             ...
             if (m_ce <= 0) {
                 if (m_Dr > 0.75) m_ce = 0.2;
                 else if (m_Dr < 0.55) m_ce = 0.5;
                 else m_ce = 0.5 - (m_Dr - 0.55) * 1.5;
                 this->console_->debug("Material {}: Parameter 'ce' was <= 0, calculating default: {}", this->id_, m_ce);
             }
         } else {
              if (m_Dr > 0.75) m_ce = 0.2;
              else if (m_Dr < 0.55) m_ce = 0.5;
              else m_ce = 0.5 - (m_Dr - 0.55) * 1.5; // Default calculation
              this->console_->debug("Material {}: Parameter 'ce' not found in JSON, calculating default: {}", this->id_, m_ce);
         }
         ...
         if (props.contains("nu")) {
             ...
             this->console_->debug("Material {}: Parameter 'nu' not found in JSON, using default: {}", this->id_, m_nu);
         }
         if (m_nu >= 0.5) {
            this->console_->debug("Material {}: Poisson's ratio nu >= 0.5 is not allowed. Setting nu = 0.499.", this->id_);
            m_nu = 0.499;
         }
         ...
         if (props.contains("Cgd")) {
             ...
             this->console_->debug("Material {}: Parameter 'Cgd' not found in JSON, using default: {}", this->id_, m_Cgd);
         }
         ...
         if (props.contains("Cdr")) {
             ...
             if (m_Cdr < 0.0) {
                 ...
                 this->console_->debug("Material {}: Parameter 'Cdr' was < 0, calculating default: {}", this->id_, m_Cdr);
             }
         } else {
             m_Cdr = std::min(10.0, (5.0 + 25.0 * (m_Dr - 0.35))); // Default calculation
             this->console_->debug("Material {}: Parameter 'Cdr' not found in JSON, calculating default: {}", this->id_, m_Cdr);
         }
         ...
         if (props.contains("Ckaf")) {
             ...
             if (m_Ckaf < 0) {
                 m_Ckaf = std::min(35.0, std::max(4.0, (5.0 + 220.0 * std::pow(std::max(0.0, m_Dr - 0.26), 3.0))));
                 this->console_->debug("Material {}: Parameter 'Ckaf' was < 0, calculating default: {}", this->id_, m_Ckaf);
             }
         } else {
             m_Ckaf = std::min(35.0, std::max(4.0, (5.0 + 220.0 * std::pow(std::max(0.0, m_Dr - 0.26), 3.0)))); // Default calculation
             this->console_->debug("Material {}: Parameter 'Ckaf' not found in JSON, calculating default: {}", this->id_, m_Ckaf);
         }
         ...
         if (props.contains("Q")) {
             ...
             this->console_->debug("Material {}: Parameter 'Q' not found in JSON, using default: {}", this->id_, m_Q);
         }
         ...
         if (props.contains("R")) {
             ...
             this->console_->debug("Material {}: Parameter 'R' not found in JSON, using default: {}", this->id_, m_R);
         }
         ...
         if (props.contains("m")) {
             ...
             this->console_->debug("Material {}: Parameter 'm' not found in JSON, using default: {}", this->id_, m_m);
         }
         ...
         if (props.contains("Fsed_min")) {
             ...
             // If user explicitly provides Fsed_min < 0, use the conditional default
              if (m_Fsed_min < 0.0) {
                 ...
                 this->console_->debug("Material {}: Parameter 'Fsed_min' was < 0, calculating default: {}", this->id_, m_Fsed_min);
             }
         } else {
             m_Fsed_min = std::min(0.99, (0.03 * std::exp(2.6 * m_Dr))); // Default calculation
             this->console_->debug("Material {}: Parameter 'Fsed_min' not found in JSON, calculating default: {}", this->id_, m_Fsed_min);
         }
         ...
         if (props.contains("p_sedo")) {
             ...
             if (m_p_sedo < 0.0) {
                 ...
                 this->console_->debug("Material {}: Parameter 'p_sedo' was < 0, calculating default: {}", this->id_, m_p_sedo);
             }
         } else {
             m_p_sedo = m_P_atm / 5.0; // Default calculation
             this->console_->debug("Material {}: Parameter 'p_sedo' not found in JSON, calculating default: {}", this->id_, m_p_sedo);
         }
         ...
         // integration_scheme: Integration scheme choice
          if (props.contains("integration_scheme")) {
             ...
             this->console_->debug("Material {}: Parameter 'integration_scheme' not found in JSON, using default: {}", this->id_, m_integrationScheme);
         }

        // Initialize p_min, p_min2 with default guesses. They will be refined based on initial stress.
        m_Pmin = m_P_atm / 200.0;
        m_Pmin2 = m_Pmin * 10.0;

     } catch (const nlohmann::json::exception& e) {
         // Catch JSON specific exceptions (like type debugs or missing keys if using .at())
         this->console_->debug("Material {}: debug reading PM4Sand properties from JSON: {}", this->id_, e.what());
         throw std::runtime_error("Failed to initialize PM4Sand parameters from JSON.");
     } catch (const std::exception& e) {
         // Catch other potential exceptions
         this->console_->debug("Material {}: Unexpected debug during PM4Sand parameter initialization: {}", this->id_, e.what());
...
mpm::dense_map PM4Sand<Tdim>::initialise_state_variables() {
    ...
    // Values will be properly set based on initial conditions during first compute_stress.
    ...
    set_scalar(&state_vars, "pzp", m_P_atm / 100.0); // Initial guess based on default p_min
    ...
    // Flag to indicate if model parameters (pmin, Mb, Md etc.) have been initialized based on stress
...
std::vector<std::string> PM4Sand<Tdim>::state_variables() const {
  std::vector<std::string> v = {
      "mean_stress", "total_vol_strain", "elastic_vol_strain", "void_ratio"};
  ...
  // scalar tails
  v.insert(v.end(), {"zcum", "zpeak", "pzp", "zxp", "pzpFlag",
                     "dGamma", "yield_state", "model_initialized"});
  return v;
}
...
void PM4Sand<Tdim>::initialise(mpm::dense_map* state_vars) {
    ...
    // The main initialization happens in the constructor and the first compute_stress call.
    (*state_vars)["yield_state"] = 0.0; // Reset yield state for the step (will be updated in compute_stress)
    (*state_vars)["dGamma"] = 0.0;      // Reset plastic multiplier increment for the step
    // No other specific initialization seems necessary here based on PM4Sand logic.
}
...
template <unsigned Tdim>
typename PM4Sand<Tdim>::Vector6d PM4Sand<Tdim>::tensor_to_voigt(const Eigen::Matrix3d& m) const {
...
typename PM4Sand<Tdim>::Vector6d PM4Sand<Tdim>::compute_stress(
    const Vector6d& stress_voigt_prev, // Stress at start of step (Voigt, Compressive Negative)
    const Vector6d& dstrain_voigt,     // Strain increment (Voigt, Engineering Shear Strain)
    ...
    if (!model_initialized) {
        ...
        if (ksi < 0.0) {
            ...
            if (m_Ado < 0) {
                 if (m_Mb >= 1.999) { m_Ado = 1.5; }
                 else {
                     double term1 = std::asin(m_Mb / 2.0); double term2 = std::asin(m_Mc / 2.0);
                     double denom = m_Mb - m_Md;
                     if (std::abs(denom) < c_tolerance) { m_Ado = 1.24; }
                     else { m_Ado = 2.5 * (term1 - term2) / denom; }
                 }
             }
        } else {
            this->m_Mb = m_Mc * std::exp(-1.0 * m_nb / 4.0 * ksi); // this->m_Mb に代入
        ...
        if (Mfin > Mcut ) {
            this->console_->debug("Material {}: Initial stress ratio M_fin={} exceeds Mcut={}. Projecting alpha.", this->id_, Mfin, Mcut);
            // initial_alpha *= (Mcut / Mfin); // 元のMPM版
            Matrix3d r = getDevPart(stress_prev) / p0 * Mcut / Mfin; // Standalone版の r 計算
            initial_alpha = r * (Mcut - m_m) / (Mcut + c_tolerance); // Standalone版の alpha 計算 (ゼロ割防止追加)
        }
        ...
        // this->console_->info("Material {}: PM4Sand model initialized. p0={:.2e}, Dr={:.2f}, pmin={:.2e}", this->id_, p0, initial_Dr, m_Pmin);
    }
    ...
    getElasticModuli(current_stress, K_elast, G_elast, Mcur_elast_dummy, current_zcum, state_vars);
    Matrix3d trialStress = current_stress + 2.0 * G_elast * dEpsilon_dev + K_elast * dEpsilon_vol * m_I;
    bool trial_yield = (getF(trialStress, current_alpha, state_vars) > -c_tolerance);
    ...
    if ( (tensordot(alpha_diff_true, n_tr) < 0.0)) {
        ...
        double p_rev = current_stress.trace() * c_one3; p_rev = std::max(p_rev, m_Pmin);
        double zxpTemp = tensorNorm(current_fabric) * p_rev;
        if (((zxpTemp > current_zxp) && (p_rev > current_pzp)) || current_pzpFlag) {
            current_zxp = zxpTemp; current_pzp = p_rev; current_pzpFlag = false;
        }
        for(int i=0; i<3; ++i) { for(int j=0; j<3; ++j) {
            if(current_alpha_in(i, j) > 0.0) { current_alpha_in_min(i,j) = std::min(current_alpha_in_min(i,j), current_alpha(i,j)); }
            else { current_alpha_in_max(i,j) = std::max(current_alpha_in_max(i,j), current_alpha(i,j)); }
        }}
        bool opposite_dir = false;
        // if (std::abs(current_alpha(0,1)) > 0.0 && std::abs(current_alpha_in_p(0,1)) > 0.0) {
        //      if (current_alpha(0,1) * current_alpha_in_p(0,1) <= 0.0) opposite_dir = true;
        // }
        if (current_alpha(0,1) * current_alpha_in_p(0,1) <= 0.0) opposite_dir = true;
        if (!opposite_dir) { // スタンドアローン版の if (m_Alpha(0, 1) * m_Alpha_in_p(0, 1) > 0) に対応
            Matrix3d alpha_in_temp = current_alpha_in; // Keep previous value temporarily if needed
            for(int i=0; i<3; ++i) { for(int j=0; j<3; ++j) {
                if(n_tr(i,j) > 0.0) { current_alpha_in(i,j) = std::max(0.0, current_alpha_in_min(i,j)); }
                else if (n_tr(i,j)<0.0){ current_alpha_in(i,j) = std::min(0.0, current_alpha_in_max(i,j)); }
                else { current_alpha_in(i,j) = std::min(0.0, current_alpha_in_max(i,j)); }
                // else { current_alpha_in(i,j) = alpha_in_temp(i,j); } // Neutral loading component, keep previous? Or set to zero? Standalone seems to keep.
            }}
    ...
    while (T < 1.0 ) {
        ...
        Matrix3d dEpsilon_sub_dev = dEpsilon_sub - (dEpsilon_sub_vol * c_one3) * m_I;

        // --- Predictor Step ---
        double K1, G1, Mcur1;
        getElasticModuli(current_stress, K1, G1, Mcur1, current_zcum, state_vars);
        double current_Dr1 = (m_emax - current_void_ratio) / (m_emax - m_emin + c_tolerance);
        current_Dr1 = std::max(0.0, std::min(current_Dr1, 1.0)); // Clamp Dr

        Matrix3d n1, R1, b1, alphaD1;
        double D1, Kp1, Cka1, h1, alphaAlphaBDotN1;
        getStateDependent(current_stress, current_alpha, current_alpha_in, current_alpha_in_p,
                          current_fabric, current_fabric_in, G1, current_zcum, current_zpeak,
                          current_pzp, Mcur1, current_Dr1,
                          n1, D1, R1, Kp1, alphaD1, Cka1, h1, b1, alphaAlphaBDotN1, state_vars);
        ...
        if (p1 < m_Pmin) {
            r1 = Matrix3d::Zero();
             ...
             if (std::abs(p1_safe) < c_tolerance) { r1 = Matrix3d::Zero(); }
             else { r1 = getDevPart(current_stress) / p1_safe; }
        }

        double denom1 = Kp1 + 2.0 * G1 - K1 * D1 * tensordot(n1, r1);
        double L1 = 0.0;
        if (std::abs(denom1) > c_tolerance) {
             L1 = (2.0 * G1 * tensordot(n1, dEpsilon_sub_dev) - tensordot(n1, r1) * K1 * dEpsilon_sub_vol) / denom1;
        }
        L1 = macauley(L1);

        // Calculate predictor increments
        Matrix3d dSigma1_e = 2.0 * G1 * dEpsilon_sub_dev + K1 * dEpsilon_sub_vol * m_I;
        ...
        if (L1 > c_tolerance && D1 < -c_tolerance) {
            if (tensordot(alphaD1 - current_alpha, n1) < 0.0) {
                 ...
                 dFabric1 = -m_cz / std::max(fab_denom, c_tolerance) * L1 * (m_z_max * n1 + current_fabric);
            }
        }
        ...
        if (p_mid < m_Pmin / 5.0) {
             if (dT <= dT_min * (1.0 + c_tolerance)) {
                 this->console_->debug("Material {}: p < p_min/5 in predictor step even at dT_min (p={}, dT={}). Proceeding.", this->id_, p_mid, dT);
             } else {
                 dT = std::max(dT_min, dT * 0.5);
                 this->console_->debug("Material {}: p < p_min/5 in predictor step (p={}). Reducing dT to {}", this->id_, p_mid, dT);
                 continue;
             }
         }
        ...
        getStateDependent(stress_mid, alpha_mid, current_alpha_in, current_alpha_in_p,
                          fabric_mid, current_fabric_in, G2, current_zcum, current_zpeak,
                          current_pzp, Mcur2, current_Dr2,
                          n2, D2, R2, Kp2, alphaD2, Cka2, h2, b2, alphaAlphaBDotN2, state_vars);
        ...
        if (p_mid < m_Pmin) {
            ...
            if (std::abs(p_mid_safe) < c_tolerance) { r2 = Matrix3d::Zero(); }
            else { r2 = getDevPart(stress_mid) / p_mid_safe; }
        }

        double denom2 = Kp2 + 2.0 * G2 - K2 * D2 * tensordot(n2, r2);
        double L2 = 0.0;
        if (std::abs(denom2) > c_tolerance) {
            L2 = (2.0 * G2 * tensordot(n2, dEpsilon_sub_dev) - tensordot(n2, r2) * K2 * dEpsilon_sub_vol) / denom2;
        }
        L2 = macauley(L2);

        // Calculate corrector increments
        Matrix3d dSigma2_e = 2.0 * G2 * dEpsilon_sub_dev + K2 * dEpsilon_sub_vol * m_I;
        ...
        if (L2 > c_tolerance && D2 < -c_tolerance) {
             if (tensordot(alphaD2 - alpha_mid, n2) < 0.0) {
                 double fab_denom = 1.0 + macauley(current_zcum / (2.0 * m_z_max) - 1.0);
                 dFabric2 = -m_cz / std::max(fab_denom, c_tolerance) * L2 * (m_z_max * n2 + fabric_mid);
             }
        }
        ...
        Matrix3d dSigma_avg = 0.5 * ((dSigma1_e - dSigma1_p) + (dSigma2_e - dSigma2_p));
        Matrix3d dSigma_diff = (dSigma2_e - dSigma2_p) - (dSigma1_e - dSigma1_p);
        double debug_estimate;
        if (stressNorm < 1.0) { debug_estimate = tensorNorm(dSigma_diff) / 2.0; }
        else { debug_estimate = tensorNorm(dSigma_diff) / (2.0 * stressNorm); }
        ...
        if (debug_estimate <= TolE || dT <= dT_min * (1.0 + c_tolerance)) {
            ...
            // current_zpeak = std::max(current_zpeak, tensorNorm(current_fabric) * c_root12);
            current_dGamma += 0.5 * (L1 + L2) * dT; // Sum of average L over substep (dT=1 assumed for L)
             max_fabric_norm_in_step = std::max(max_fabric_norm_in_step, tensorNorm(current_fabric) * c_root12); // ステップ中の最大ノルムを更新
            double p_next_sub = current_stress.trace() * c_one3;
            if (p_next_sub < m_Pmin / 5.0) {
                this->console_->debug("Material {}: p < p_min/5 after substep update (p={}). Clamping stress.", this->id_, p_next_sub);
            ...
            dT *= q;
            dT = std::max(dT_min, dT);
            this->console_->debug("Material {}: Substep rejected (debug={:.2e}). Reducing dT to {}", this->id_, debug_estimate, dT);
        }
        substep_iter++;

    } // End while (T < 1.0)

    // ... (Keep the rest of the function: post-loop check, stress correction call, state update, return value) ...
    if (substep_iter >= max_substeps && T < 1.0 - c_tolerance) {
         this->console_->debug("Material {}: Modified Euler integration failed to converge within max substeps (T={}). Using last calculated state.", this->id_, T);
    }
    ...
    double F_yield_final = getF(nextStress, nextAlpha, state_vars);
     if (F_yield_final > c_tolerance * 10.0 || nextStress.trace() * c_one3 < m_Pmin / 5.0) {
         if (F_yield_final > c_tolerance * 10.0)
             this->console_->debug("Material {}: Yield condition violated after full step integration (F={:.2e} > {:.2e}). Applying stress correction.", this->id_, F_yield_final, c_tolerance * 10.0);
         else
             this->console_->debug("Material {}: p < p_min/5 after full step integration (p={:.2e}). Applying stress correction.", this->id_, nextStress.trace() * c_one3);

         stressCorrection(nextStress, nextAlpha, stress_prev,
                          alpha_in_n, alpha_in_p_n, // Use alpha history from start of step
         ...
         this->console_->debug("Material {}: Stress correction finished (F={:.2e}, p={:.2e}).", this->id_, F_yield_final, nextStress.trace() * c_one3);
     }

    // -------------------------------------------------
    // 6. Update state variables for storage (Commit State)
    // -------------------------------------------------
    current_zcum = initial_zcum + accumulated_dFabricNorm; // ステップ終了時に更新
    current_zpeak = std::max(initial_zpeak, max_fabric_norm_in_step); // ステップ終了時に更新 (ステップ中の最大 Fabric ノルムと初期値を比較)
...
typename PM4Sand<Tdim>::Matrix6x6 PM4Sand<Tdim>::compute_elasto_plastic_tensor(
    ...
    const ParticleBase<Tdim>* /*ptr*/, dense_map* state_vars,double dt, bool /*hardening*/) {
    ...
    // differentiation of the stress update algorithm, which is complex for PM4Sand.
    return De;
}
...
double PM4Sand<Tdim>::getKsi(double Dr, double p) {
    ...
    if (std::abs(denom) < c_tolerance) {
         return (denom > 0 ? 100.0 : -100.0); // Indicate near critical state singularity
    }
    return m_R / denom - Dr;
}

template <unsigned Tdim>
void PM4Sand<Tdim>::getElasticModuli(const Matrix3d& sigma, double& K, double& G, dense_map* state_vars) {
...
template <unsigned Tdim>
void PM4Sand<Tdim>::getElasticModuli(const Matrix3d& sigma, double& K, double& G, double& Mcur, const double& zcum, dense_map* state_vars) {
    ...
    Mcur = (pn < c_tolerance) ? 0.0 : (std::sqrt(1.5) * tensorNorm(deviatoricStress) / pn);
    ...
    current_void_ratio = std::max(m_emin * 0.9, std::min(current_void_ratio, m_emax * 1.1));
    double current_Dr = (m_emax - current_void_ratio) / (m_emax - m_emin + c_tolerance);
    current_Dr = std::max(0.0, std::min(current_Dr, 1.0)); // Clamp Dr between 0 and 1
    ...
    // Clamp Mb if it becomes excessively large (can happen with very dense states/low p)


    double Csr = 1.0 - Csr0 * std::min(1.0, std::pow(Mcur / (Mb_current+ c_tolerance), msr)); // <<< this->m_Mb を使用
    ...
    if (postShake) {
        double Md_current;
        if (ksi <= 0.0) Md_current = m_Mc * std::exp(m_nd * ksi);
        else Md_current = m_Mc * std::exp(m_nd * 4.0 * ksi);

        double p_sed = m_p_sedo * (zcum / (zcum + m_z_max + c_tolerance)) * std::pow(macauley(1.0 - Mcur / (Md_current + c_tolerance)), 0.25);
        double F_sed = std::min(m_Fsed_min + (1.0 - m_Fsed_min) * (pn / (20.0 * (p_sed + c_tolerance))), 1.0);
...
template <unsigned Tdim>
Eigen::Matrix3d PM4Sand<Tdim>::getNormalToYield(const Matrix3d& stress, const Matrix3d& alpha, dense_map* /*state_vars*/) {
     // Implementation matching PM4Sand_claude_2.cpp logic
     double p = stress.trace() * c_one3;
     Matrix3d n;
     Matrix3d s = getDevPart(stress);

     if (p < m_Pmin * 1.1) { // Use a slightly larger threshold for stability near p_min
        ...
        //  }
         ...
         Matrix3d s_eff = s - p * alpha; // Effective deviator relative to yield surface center
...
template <unsigned Tdim>
double PM4Sand<Tdim>::getF(const Matrix3d& stress, const Matrix3d& alpha, dense_map* /*state_vars*/) {
    // Implementation matching PM4Sand_claude_2.cpp logic
    Matrix3d s = getDevPart(stress);
    double p = stress.trace() * c_one3;
    ...
    Matrix3d s_eff = s - p * alpha; // Effective deviator relative to yield surface center
    // Yield function: f = || s - p*alpha || - sqrt(2/3) * m * p  (Note: Original uses sqrt(1/2))
    // Let's stick to sqrt(1/2) as per original code logic check
...
void PM4Sand<Tdim>::getStateDependent(
    const Matrix3d& stress, const Matrix3d& alpha,
    const Matrix3d& alphaIn, const Matrix3d& alphaInP,
    const Matrix3d& fabric, const Matrix3d& fabricIn,
    double& G, // Input G
    const double& zcum, const double& zpeak, const double& pzp, // Input state scalars
    ...
    Matrix3d alphaB = c_root12 * (Mb_current - m_m) * n; // Use sqrt(1/2) consistent with getF
    ...
    double Cpzp2 = macauley(pzp - p) / (macauley(pzp - p) + m_Pmin + c_tolerance); // Use m_Pmin2
    ...
    Matrix3d alpha_in_true = get_tensor(state_vars, "alpha_in_true"); // Get from state_vars
    Matrix3d alphaAlphaInTrue = alpha - alpha_in_true;
    double alphaAlphaInTrueDotN = macauley(tensordot(alphaAlphaInTrue, n));

    // Calculate Cka
    Cka = 1.0 + m_Ckaf / (1.0 + std::pow(2.5 * alphaAlphaInTrueDotN, 2.0)) * Cpzp2 * Czpk1; // Output Cka
    ...
    double h_denom_czpk = (1.0 + Ckp * zpeak / zmax_safe * macauley(alphaAlphaBDotN) * std::sqrt(std::max(0.0, 1.0 - Czpk2)));
    ...
    if (std::sqrt(std::abs(alphaAlphaBDotN)) < c_tolerance ) {
        ...
        if (tensordot(alphaAlphaP, n) <= 0) {
             h *= (alphaAlphaInDotN + Cg1) / (alphaAlphaInTrueDotN + Cg1 + c_tolerance);
        }
    }
    ...
    // Check PM4Sand code: Kp = 2/3 * h * p * tensordot(b, n) - yes, sqrt is not there.
    K_p = c_two3 * h * p * tensordot(b, n);
    K_p = std::max(0.0, K_p); // Ensure non-negative
    // Output K_p

    // --- Calculate dilatancy D ---
    double Czin1 = macauley(1.0 - std::exp(-2.0 * std::abs(tensordot(fabricIn, n) - tensordot(fabric, n)) / (zmax_safe + c_tolerance)));

    // Rotated dilatancy surface calculation
    Matrix3d minusFabric = -fabric;
    double Crot1_term = macauley(tensordot(minusFabric, n));
    // double Crot1 = 1.0 + 2.0 * Crot1_term / ( sqrt(2.0)* zmax_safe + c_tolerance) * (1.0 - Czin1); // 元のMPM版
    double Crot1 = std::max((1.0 + 2.0 * Crot1_term / (sqrt(2.0) * zmax_safe + c_tolerance) * (1.0 - Czin1)), 1.0); // Standalone版に合わせる (max と sqrt(2.0) の位置修正)
    
    double Mdr = Md_current / (Crot1 + c_tolerance); // Rotated dilatancy stress ratio
    ...
    if (tensordot(alphaDrAlpha, n) <= 0) {
        // Dilation regime
        double Cpzp = (pzp < c_tolerance) ? 1.0 : 1.0 / (1.0 + std::pow(2.5 * p / (pzp + c_tolerance), 5.0));
        double Cpmin = 1.0 / (1.0 + std::pow(m_Pmin2 / (p + c_tolerance), 2.0));
        double Czin2 = (1.0 + Czin1 * (zcum - zpeak) / (3.0 * zmax_safe + c_tolerance)) /
                       (1.0 + 3.0 * Czin1 * (zcum - zpeak) / (3.0 * zmax_safe + c_tolerance) + c_tolerance);

        double zpeak_safe = std::max(zpeak, c_tolerance);
        double temp_fab = std::pow(macauley(1.0 - Crot1_term * c_root12 / zpeak_safe), 3.0); // sqrt(1/2) here
        double Ad_denom = (std::pow(zcum_safe, 2.0) / zmax_safe) * temp_fab * std::pow(m_ce, 2.0) * Cpzp * Cpmin * Czin1 + 1.0;
        double Ad = m_Ado * Czin2 / (Ad_denom + c_tolerance);

        D = Ad * tensordot(alphaD_alpha, n);

        // Rotation effect on dilatancy
        double Drot = Ad * Crot1_term / (sqrt(2.0) * zmax_safe + c_tolerance) * // sqrt(1/2) here
                      tensordot(alphaDrAlpha, n) / (m_Cdr + c_tolerance);

        if (D > Drot) {
            D = D + (Drot - D) * macauley(Mb_current - Mcur) / (macauley(Mb_current - Mcur) + 0.01 + c_tolerance);
        }

        // Limit dilatancy at very low pressure
        if (m_Pmin <= p && p <= 2.0 * m_Pmin) {
            D = std::min(D, -3.5 * m_Ado * macauley(Mb_current - Md_current) * (2.0 * m_Pmin - p) / (m_Pmin + c_tolerance));
        }
    } else {
        // Contraction regime
        ...
        // double Cdz_term1 = (1.0 - Crot2 * sqrt(2.0)  * zpeak / zmax_safe); // 元のMPM版の一部
        // double Cdz_term2 = (zmax_safe / (zmax_safe + Crot2 * zcum + c_tolerance));
        // double Cdz = std::max(Cdz_term1 * Cdz_term2, 1.0 / (1.0 + zmax_safe / 2.0 + c_tolerance)); // 元のMPM版の一部
        double Cdz = std::max((1.0 - Crot2 * sqrt(2.0) * zpeak / zmax_safe) * (zmax_safe / (zmax_safe + Crot2 * zcum + c_tolerance)),
                              1.0 / (1.0 + zmax_safe / 2.0 + c_tolerance)); // Standalone版の計算式全体を実装
        Cdz = std::max(c_tolerance, Cdz); // Ensure positive
        double Adc = m_Ado * (1.0 + macauley(tensordot(fabric, n))) / (hp * Cdz + c_tolerance);

        double Cin = 2.0 * macauley(tensordot(fabric, n)) / sqrt(2.0) / (zmax_safe + c_tolerance);
...
void PM4Sand<Tdim>::stressCorrection(
    ...
    const Matrix3d& curFabric, double& nextVoidRatio, dense_map* state_vars) {
        Matrix3d dSigmaP, dfrOverdSigma, dfrOverdAlpha, n_iter, R_iter, alphaD_iter, b_iter, r_iter,aBar;
        Matrix3d nAlpha = nextAlpha; // Use local copies for iteration
        Matrix3d nStress = nextStress;
        double lambda, D_iter, Kp_iter, Cka_iter, h_iter, p_iter, fr, alphaAlphaBDotN_iter;
        double K_iter, G_iter, Mcur_iter; // Elastic moduli

        // Check for small p (Function 2 logic)
        p_iter = nStress.trace() * c_one3;
        fr = getF(nStress, nAlpha, state_vars); // Calculate fr regardless of p initially

        if (p_iter < m_Pmin / 5.0) {
             // this->console_->debug("Material {}: Stress correction initial p < p_min/5 ({:.2e})", this->id_, p_iter);
            if (fr < c_tolerance) {
                // Stress state inside yield surface or on it - adjust p only
                nextStress = getDevPart(nStress)  + (m_Pmin / 5.0 - p_iter) * m_I; // Function 2 direct addition
                // Alternative: preserve deviatoric part
                // Matrix3d s_curr = getDevPart(nStress);
                // nextStress = s_curr + (m_Pmin / 5.0) * m_I;
                // this->console_->debug("Material {}: Stress adjusted to p_min/5 while inside yield.", this->id_);
                nextStress = nStress;
                nextAlpha = nAlpha; // nAlpha はこの分岐では変更されていない
                return;
            } else {
                // Stress state outside yield surface - reset (Function 2 specific reset)
                // this->console_->debug("Material {}: Stress reset to specific state due to p < p_min/5 and outside yield.", this->id_);
                nextStress = (m_Pmin / 5.0) * m_I;
                // Function 2 specific shear component reset (might need review for general cases)
        ...
        double current_Dr = (m_emax - nextVoidRatio) / (m_emax - m_emin + c_tolerance);
        current_Dr = std::max(0.0, std::min(current_Dr, 1.0));

        double dGamma_step = get_scalar(state_vars, "dGamma");
        // Function 2 doesn't explicitly handle dGamma <= 0 here, assuming it's positive from the integration step.
        // If dGamma_step could be zero/negative, add handling if necessary.
         if (dGamma_step <= 0) {
             // Option: Use small value like Function 1, or return/throw debug if invalid state
             // this->console_->debug("Material {}: Stress correction called but dGamma<=0. Correction may be ineffective.", this->id_);
        ...
        Matrix3d nStress_newton_start = nStress; // Newton-Raphson開始時の応力（二分法の参照用）
        ...
        for (int i = 1; i <= maxIter; i++) {
            ...
            // Calculate stress ratio tensor r (Function 2 logic)
             r_iter = getDevPart(nStress) / p_iter; // Division by p_iter (already max(p, p_min))
            //  r_iter = (std::abs(p_iter) < c_tolerance) ? Matrix3d::Zero() : getDevPart(nStress) / p_iter;
            ...
            getStateDependent(nStress, nAlpha, alphaIn, alphaInP, curFabric, fabric_in_iter,
                              G_iter, zcum_iter, zpeak_iter, pzp_iter, Mcur_iter, current_Dr,
                              n_iter, D_iter, R_iter, Kp_iter, alphaD_iter, Cka_iter, h_iter, b_iter,
                              alphaAlphaBDotN_iter, state_vars);

            // Calculate terms for correction (Function 2 structure)
            // NOTE: Function 2 used m_dGamma implicitly. We use dGamma_step from state_vars.
            if (dGamma_step > c_tolerance * 1e-2) { // dGammaが意味のある大きさの場合のみdSigmaPを計算
                dSigmaP = dGamma_step * (2.0 * G_iter * n_iter + K_iter * D_iter * m_I); // R_iterではなくn_iterを使用するスタンドアローン版のdSigmaPの計算方法に近づけるか検討が必要。PM4Sand.cpp(OpenSees)ではRを使っている。ここでは元のMPM版のR_iterのままとする。
                // dSigmaP = dGamma_step * (2.0 * G_iter * R_iter + K_iter * D_iter * m_I); // 元のMPM版
           } else {
                dSigmaP = Matrix3d::Zero(); // dGammaが小さすぎる場合は塑性的な応力変化は小さいと見なす
           }
            // dSigmaP = dGamma_step * (2.0 * G_iter * R_iter + K_iter * D_iter * m_I); // R_iter used in Func2 dSigmaP calc
            aBar = (c_two3 * h_iter * b_iter); // Same as Function 1 basically

            dfrOverdSigma = n_iter - 0.5 * tensordot(n_iter, r_iter) * m_I; // Function 2 gradient
            dfrOverdAlpha = -p_iter * n_iter;             // Function 2 gradient

            // Calculate lambda (Function 2 approach - no stabilization)
            double denom_lambda = tensordot(dfrOverdSigma, dSigmaP) - tensordot(dfrOverdAlpha, aBar);

            if (std::abs(denom_lambda) < c_tolerance * 1e-5) { // 分母がゼロに近い場合の処理
                // this->console_->debug("Material {}: Stress correction lambda denominator near zero ({:.2e}). Using alternative lambda. Iter: {}", this->id_, denom_lambda, i);
                double lambda_alt_denom = tensordot(dfrOverdSigma, dfrOverdSigma);
                if (std::abs(lambda_alt_denom) < c_tolerance * 1e-5) {
                    // this->console_->warn("Material {}: Stress correction alternative lambda denominator also near zero. Aborting NR. Iter: {}", this->id_, i);
            ...
            if (std::abs(fr) < c_tolerance) { // Use Function 2 tolerance
                // this->console_->debug("Material {}: Stress correction converged in {} iterations (F={:.2e}).", this->id_, i, fr);
                converged = true;
                nextStress = nStress;
                nextAlpha = nAlpha;
             ...
             // Update p_iter for next iteration's r_iter calculation (though loop might end here)
             // p_iter = std::max(c_one3 * nStress.trace(), m_Pmin); // Redundant? Already calculated at loop start.

        } // End Newton-Raphson correction loop

        // --- Bisection Search if Newton-Raphson didn't converge (Function 2 logic) ---
        if (!converged) {
            Matrix3d dSigma_bisection = nStress_newton_start - stress_prev; // Difference between initial state needing correction and pre-increment state
            double alpha_L = 0.0; // 補正なし（つまり stress_before_increment + 0.0 * dSigma_bisection = stress_before_increment）
            double alpha_R = 1.0; // 完全な増分（つまり stress_before_increment + 1.0 * dSigma_bisection = nStress_newton_start）
            double fr_L = getF(stress_prev + alpha_L * dSigma_bisection, nAlpha_newton_start, state_vars);
            double fr_R = getF(stress_prev + alpha_R * dSigma_bisection, nAlpha_newton_start, state_vars);
            if (fr_L * fr_R > 0 && std::abs(fr_R) > c_tolerance) { // 解を挟んでいない、かつ右端がすでに降伏面を超えている
                // this->console_->warn("Material {}: Bisection range does not bracket the root initially (fL={:.2e}, fR={:.2e}). Check bisection reference states.", this->id_, fr_L, fr_R);
            ...
            for (int jj = 0; jj < maxBisectionIter; jj++) {
                ...
                Matrix3d stress_mid_bisection = stress_prev + alpha_mid * dSigma_bisection;
                double fr_mid = getF(stress_mid_bisection, nAlpha_newton_start, state_vars);

                if (std::abs(fr_mid) < c_tolerance) {
                    nStress = stress_mid_bisection;
                    nAlpha = nAlpha_newton_start; // αは二分法では変更しないのが一般的
                    converged = true;
                    this->console_->debug("Material {}: Bisection converged in {} iterations (F_mid={:.2e}).", this->id_, jj + 1, fr_mid);
                    break;
                }
                ...
                if ((alpha_R - alpha_L) < 1e-6) { // 収束幅が非常に小さい
                    this->console_->debug("Material {}: Bisection interval too small at iteration {}. F_mid={:.2e}", this->id_, jj + 1, fr_mid);
...
// Explicit instantiation for 2D and 3D to avoid linking debugs if used elsewhere
// If the class is only used with Tdim=2, only instantiate for 2.
// If the framework might instantiate with Tdim=3, include 3 as well.
