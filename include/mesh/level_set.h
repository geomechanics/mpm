...
//! LevelSet class
//! \brief Base class that stores the information about level-sets
//! \details LevelSet class which stores the level-set value and gradient
...
class LevelSet {
 public:
  ...
  // Construct a LevelSet with a global unique id
  //! \param[in] id Global LevelSet id
  //! \param[in] isoparametric LevelSet is isoparametric
  LevelSet(unsigned id, bool isoparametric = true);
 ...
 LevelSet(const LevelSet<Tdim>&) = delete;
  ...
  //! Return id of the LevelSet
  unsigned id() const { return id_; }

  //! Return if a LevelSet is isoparametric
  ...
  //! Remove a node from the LevelSet
  //! \param[in] node A shared pointer to node
  //! \retval insertion_status Return the successful addition of a node
  ...
  //! Create a list of active nodes in LevelSet
  ...
  //! Add a particle to the LevelSet
  ...
  //! Remove a particle from the LevelSet
  //! \param[in] particle A shared pointer to particle
  //! \retval insertion_status Return the successful addition of a particle
  ...
  //! Find shared nodes across MPI domains in the LevelSet
  ...
  //! Number of particles in the LevelSet
  mpm::Index nparticles() const { return particles_.size(); }

  //! Number of particles in the LevelSet with specific type
  ...
  //! Iterate over all cells in a LevelSet to find the cell in which particles
  //! are located.
  //! \retval particles Particles which cannot be located in the LevelSet
  std::vector<std::shared_ptr<mpm::ParticleBase<Tdim>>> locate_particles_levelset();
  ...
  //! Return status of the LevelSet. A LevelSet is active, if at least one particle is
  //! present
  ...
  //! Add a neighbour LevelSet, using the local id for the new LevelSet and a LevelSet
  //! pointer
  //! \param[in] local_id local id of the LevelSet
  //! \param[in] neighbour A shared pointer to the neighbouring LevelSet
  //! \retval insertion_status Return the successful addition of a node
  bool add_neighbour(unsigned local_id,
                     const std::shared_ptr<LevelSet<Tdim>>& neighbour);

  //! Return the number of neighbouring LevelSets
  unsigned nneighbours() const { return neighbour_levelsets_.size(); }
  ...
  //! Create a list of active nodes in LevelSet and assign active node id
  //! (rank-wise)
  //! \ingroup MultiPhase
 ...
 private:
  ...
  // Locate a particle in LevelSet cells
 ...
 private:
  ...
  //! Isoparametric LevelSet
  bool isoparametric_{true};
  //! Vector of LevelSet neighbours
  Map<LevelSet<Tdim>> neighbour_levelsets_;
  //! Vector of particles
...
#include "level_set.tcc"

#endif  // MPM_LEVELSET_H_
