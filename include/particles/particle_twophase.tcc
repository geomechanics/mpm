...
template <unsigned Tdim>
void mpm::TwoPhaseParticle<Tdim>::map_liquid_mass_momentum_to_nodes(
    mpm::VelocityUpdate velocity_update) noexcept {
...
template <unsigned Tdim>
void mpm::TwoPhaseParticle<
    Tdim>::map_liquid_mass_momentum_to_nodes_taylor() noexcept {
  ...
  for (unsigned i = 0; i < nodes_.size(); ++i) {
    ...
    map_velocity.noalias() += mapping_matrix_liquid_ *
...
template <unsigned Tdim>
void mpm::TwoPhaseParticle<Tdim>::compute_updated_liquid_velocity_tpic(
    double dt) noexcept {
  // Check if particle has a valid cell ptr
  assert(cell_ != nullptr);

  // Compute auxiliary mapping matrix
  mapping_matrix_liquid_ =
      this->compute_velocity_gradient(this->dn_dx_, mpm::ParticlePhase::Liquid);
...
bool mpm::TwoPhaseParticle<Tdim>::map_ppp_stabilization_matrix(double dt) {
  ...
  try {
    ...
    const double E =
        (state_variables_[mpm::ParticlePhase::Solid].find("youngs_modulus") !=
         state_variables_[mpm::ParticlePhase::Solid].end())
