...
inline const Eigen::Matrix<double, 2, 2> mpm::math::matrix_form<2>(
  ...
  matrix_tensor(0, 1) = shear_multiplier * voigt_tensor(3);
...
inline const Eigen::Matrix<double, 3, 3> mpm::math::matrix_form<3>(
    const Eigen::Matrix<double, 6, 1>& voigt_tensor, double shear_multiplier) {
  return mpm::math::matrix_form(voigt_tensor, shear_multiplier);
}
...
inline const Eigen::Matrix<double, 3, 3> mpm::math::matrix_form(
  ...
  matrix_tensor(0, 1) = shear_multiplier * voigt_tensor(3);
  matrix_tensor(1, 0) = matrix_tensor(0, 1);
  matrix_tensor(1, 2) = shear_multiplier * voigt_tensor(4);
  matrix_tensor(2, 1) = matrix_tensor(1, 2);
  matrix_tensor(0, 2) = shear_multiplier * voigt_tensor(5);
...
inline const Eigen::Matrix<double, 6, 1> mpm::math::voigt_form<1>(
    const Eigen::Matrix<double, 1, 1>& matrix_tensor, double shear_multiplier) {
...
inline const Eigen::Matrix<double, 6, 1> mpm::math::voigt_form<2>(
    const Eigen::Matrix<double, 2, 2>& matrix_tensor, double shear_multiplier) {
  Eigen::Matrix<double, 6, 1> voigt_tensor =
      Eigen::Matrix<double, 6, 1>::Zero();
  voigt_tensor(0) = matrix_tensor(0, 0);
  voigt_tensor(1) = matrix_tensor(1, 1);

  voigt_tensor(3) = shear_multiplier * matrix_tensor(0, 1);
  return voigt_tensor;
}
...
inline const Eigen::Matrix<double, 6, 1> mpm::math::voigt_form<3>(
  ...
  return mpm::math::voigt_form(matrix_tensor, shear_multiplier);
}
...
inline const Eigen::Matrix<double, 6, 1> mpm::math::voigt_form(
  ...
  voigt_tensor(3) = shear_multiplier * matrix_tensor(0, 1);
  voigt_tensor(4) = shear_multiplier * matrix_tensor(1, 2);
  voigt_tensor(5) = shear_multiplier * matrix_tensor(0, 2);
  return voigt_tensor;
}
...
inline const Eigen::Matrix<double, 3, 1> mpm::math::principal_tensor(
  ...
  Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> es(matrix_tensor);
