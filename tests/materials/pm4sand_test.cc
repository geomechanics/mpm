...
TEST_CASE("PM4Sand undrained cyclic test in 2D", "[material][PM4Sand][2D]") {
  ...
  double initial_confining_pressure =
      100000.0;  // Pa (use positive for calculation, apply sign later)
  double static_tau = 0.0;  // Initial static shear stress (Pa)

  // Json for PM4Sand material properties (using default values from
  ...
  // Add other parameters (h0, emax, emin, nb, nd etc.) or rely on defaults in
  // initialise_parameters
  ...
  double c_pin =
      -initial_confining_pressure;  // Compressive pressure is negative
  Eigen::Matrix3d stres_tensor =
      Eigen::Matrix3d::Zero();  // Use 3D tensor internally
  ...
  stres_tensor(2, 2) =
      c_pin;  // sigma_zz approx K0*sigma_yy (often needed for p calculation)
  stres_tensor(0, 1) = -static_tau;  // Apply static shear (negative to match
  ...
  stress_initial << stres_tensor(0, 0), stres_tensor(1, 1), stres_tensor(2, 2),
      stres_tensor(0, 1), stres_tensor(1, 2),
      stres_tensor(2, 0);  // yz=0, zx=0 for 2D

  // --- Strain Increment ---
  // Apply simple shear strain increment gamma_xy
  double dstrain_shear_val =
      -2e-6;  // Small engineering shear strain increment gamma_xy
  mpm::Material<Dim>::Vector6d dstrain = mpm::Material<Dim>::Vector6d::Zero();
  dstrain(3) =
      dstrain_shear_val;  // Apply gamma_xy to the xy component (index 3)

  // --- State Variables ---
  mpm::dense_map state_vars = material->initialise_state_variables();
  // Initialise may perform checks or set flags - call it after getting initial
  // state_vars
  material->initialise(&state_vars);

  // --- Simulation Loop Setup ---
  std::vector<std::array<double, 3>> result;  // [eps_xy (%), p (kPa), tau_xy
                                              // (kPa)]
  // Store initial state
  result.push_back({0.0,  // Initial shear strain
                    -stress_initial.head(3).sum() / 3.0 /
                        1000.0,  // Initial p (kPa, compression positive)
  ...
  double cumulative_strain_xy =
      0.0;  // Cumulative *tensor* shear strain epsilon_xy
  ...
  while (ireverse < nreverse * 2 && istep < max_steps) {  // Loop over reversals
    ...
    auto updated_stress_voigt = material->compute_stress(
        current_stress, dstrain, particle.get(), &state_vars);
    ...
    cumulative_strain_xy += -dstrain(3) * 0.5;  // Accumulate tensor strain
                                                // epsilon_xy = gamma_xy / 2

    // Calculate results (convert back to desired units/sign convention)
    double p_now_kPa = -current_stress.head(3).sum() / 3.0 /
                       1000.0;  // Mean stress (kPa, compression positive)
    double tau_xy_kPa = -current_stress(3) / 1000.0;  // Shear stress (kPa)
    double eps_xy_percent =
        cumulative_strain_xy * 2.0 *
        100.0;  // Engineering shear strain (%) gamma_xy = 2 * epsilon_xy
    ...
    if (std::abs(current_stress(3)) > max_tau_abs) {
      ...
      std::cout << "Reversal #: " << ireverse << " at step " << istep
                << ", tau_xy = " << current_stress.head(3) << " kPa"
                << std::endl;
      // Optional: Add check here if needed
      // REQUIRE(std::fabs(current_stress(3)) >= max_tau_abs * (1.0 -
      // Tolerance));
    }
