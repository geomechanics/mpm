...
//! \brief Check particle levelset bbar class for 2D case
TEST_CASE("ParticleLevelsetBbar is checked for 2D case",
          "[particle][2D][Levelset][Bbar]") {
  // Dimension
  const unsigned Dim = 2;
  // Degree of freedom
  const unsigned Dof = 2;
  ...
  SECTION("Test levelset particle, cell and node functions") {
    ...
    // Particle type
    REQUIRE(particle->type() == "P2DLSBBAR");
...
//! \brief Check particle levelset bbar class for 3D case
TEST_CASE("ParticleLevelsetBbar is checked for 3D case",
          "[particle][3D][Levelset][Bbar]") {
  // Dimension
  const unsigned Dim = 3;
  // Dimension
  const unsigned Dof = 6;
