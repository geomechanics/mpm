...
// Check MPM Explicit Acceleration Constraint
TEST_CASE("MPM 2D Explicit Acceleration Constraint is checked",
          "[MPM][2D][Explicit][Acceleration]") {
  // Dimension
  ...
  char* argv[] = {(char*)"./mpm",
                  (char*)"-f",  (char*)"./",
                  (char*)"-i",  (char*)"mpm-explicit-acceleration-2d.json"};
  // clang-format on

  SECTION("Check initialisation") {
    const bool resume = false;
    const unsigned dir = 1;
    REQUIRE(mpm_test::write_json_acceleration(2, resume, analysis, fname,
                                              dir) == true);
...
// Check MPM Explicit Acceleration Constraint
TEST_CASE("MPM 3D Explicit Acceleration Constraint is checked",
          "[MPM][3D][Explicit][Acceleration]") {
  // Dimension
  ...
  SECTION("Check initialisation") {
    const bool resume = false;
    const unsigned dir = 1;
    REQUIRE(mpm_test::write_json_acceleration(3, resume, analysis, fname,
                                              dir) == true);
