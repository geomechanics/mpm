...
// Check MPM Explicit Velocity Constraint
TEST_CASE("MPM 2D Explicit Velocity Constraint is checked",
          "[MPM][2D][Explicit][Velocity]") {
  // Dimension
  const unsigned Dim = 2;

  // Write JSON file
  const std::string fname = "mpm-explicit-velocity";
  ...
  char* argv[] = {(char*)"./mpm",
                  (char*)"-f",  (char*)"./",
                  (char*)"-i",  (char*)"mpm-explicit-velocity-2d.json"};
  // clang-format on

  SECTION("Check initialisation") {
    const bool resume = false;
    const unsigned dir = 1;
    REQUIRE(mpm_test::write_json_velocity(2, resume, analysis, fname, dir) ==
            true);
...
// Check MPM Explicit Velocity Constraint
TEST_CASE("MPM 3D Explicit Velocity Constraint is checked",
          "[MPM][3D][Explicit][Velocity]") {
  // Dimension
  ...
  char* argv[] = {(char*)"./mpm",
                  (char*)"-f",  (char*)"./",
                  (char*)"-i",  (char*)"mpm-explicit-velocity-3d.json"};
  // clang-format on

  SECTION("Check initialisation") {
    const bool resume = false;
    const unsigned dir = 1;
    REQUIRE(mpm_test::write_json_velocity(3, resume, analysis, fname, dir) ==
            true);
